...
function cares_setup() {
	...
	if ( function_exists( 'add_image_size' ) ) {
		...
		add_image_size( 'featured-desktop', 1024, 400 ); //(cropped)
		// Used for responsive images, eh? See inc/template-tags.php->cares_responsive_thumbnail() for implementation
		add_image_size( 'featured-300', 300, 200 ); //(cropped)
		add_image_size( 'featured-450', 450, 225); //(cropped)
		add_image_size( 'featured-600', 600, 300 ); //(cropped)
		add_image_size( 'featured-800', 800, 350  ); //(cropped)
		//add_image_size( 'featured-800', 800 ); //(cropped)
	}
...
function cares_scripts() {
	$theme_version = cares_theme_version();

	wp_enqueue_script( 'jquery' );
	wp_enqueue_style( 'cares-style', get_stylesheet_uri(), array(), $theme_version );
	wp_enqueue_script( 'cares-navigation', get_template_directory_uri() . '/js/navigation.js', array(), $theme_version, true );
	...
	wp_enqueue_script( 'cares-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), $theme_version, true );
...
// Use the Custom Content Portfolio function to fetch the CPT name.
function cares_get_portfolio_cpt_name() {
	$cpt = 'portfolio_project';
	if ( function_exists( 'ccp_get_project_post_type' ) ) {
		$cpt = ccp_get_project_post_type();
	}
	return $cpt;
}